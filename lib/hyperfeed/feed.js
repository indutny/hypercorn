...
Feed.prototype.watch = function watch(range, callback) {
  ...
  const watcher = this.node.watch({
    start: this._meta(this._postKey(range.start)),
    end: range.end && this._meta(this._postKey(range.end))
  });

  return {
    post: this.hypercore.createReadStream({
      start: range.start,
      end: range.end,
      live: !range.end,
      wait: true
...
Feed.prototype.append = function append(post) {
  assert(this.writable, 'Feed not writable');
  this.hypercore.append(post);
};
...
Feed.prototype._postKey = function _postKey(index) {
  // Pad to 32-bit hex value
  let str = index.toString(16);
  while (str.length < 8)
    str = '0' + str;
  return POST_PREFIX + str;
};
